...
func (h *SeckillHandler) Handle(orderCtx *OrderContext) error {
	productID := orderCtx.Request.URL.Query().Get("seckill_product_id")
	if productID == "" {
	...
	span.SetAttributes(
		...
		attribute.String("user.id", orderCtx.UserId),
	)

	fmt.Println("【责任链】=> 步骤 0.5: 秒杀资格校验 (调用 SeckillService)...")

	// ✨ [核心改造] 调用 SeckillService 处理业务逻辑
	resultCode, err := h.seckillService.AttemptSeckill(ctx, productID, orderCtx.UserId)
	if err != nil {
	...
	case ResultSuccess:
		...
		fmt.Println("【秒杀】=> 用户", orderCtx.UserId, "成功抢到商品", productID)
		return h.executeNext(orderCtx)
	case ResultSoldOut:
	...
	case ResultAlreadyPurchased:
		...
		return fmt.Errorf("user %s already purchased product %s", orderCtx.UserId, productID)
	default:
